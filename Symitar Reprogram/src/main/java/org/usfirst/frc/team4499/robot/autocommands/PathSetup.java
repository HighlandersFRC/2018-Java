...
public class PathSetup {
    ...
    public Trajectory generateMainPath(){
        ...
        Trajectory.Config config = new Trajectory.Config(Trajectory.FitMethod.HERMITE_CUBIC, Trajectory.Config.SAMPLES_LOW, 0.05,8,RobotConfig.maxAcceleration, 75.0);
    ...
    public DistanceFollower generateLeftPathFollower(){
        ...
        //then (C=2piR)/x and solve for R then double I guess units aren't cleared and I kinda just increased it till it seemed to work
        TankModifier modifier = new TankModifier(generateMainPath()).modify(3.12);
