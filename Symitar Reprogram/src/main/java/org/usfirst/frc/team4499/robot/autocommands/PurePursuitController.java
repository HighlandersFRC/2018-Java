...
import org.usfirst.frc.team4499.robot.RobotMap;
import org.usfirst.frc.team4499.robot.tools.Point;
import org.usfirst.frc.team4499.robot.RobotConfig;
import org.usfirst.frc.team4499.robot.tools.Vector;
...
public class PurePursuitController extends Command {
  ...
  private class PathRunnable implements Runnable{
    public void run(){
      ...
      for(int i = startingNumberLA; i<chosenPath.getMainPath().length()-1;i++){
        ...
        if(discriminant<0){
          ...
          if(lookAheadIndexT1>=0&&lookAheadIndexT1<=1){
            ...
            if(partialPointIndex>lastPointIndex){
              lookAheadPoint.setLocation(startingPointOfLineSegment.getXPos() + lookAheadIndexT2*lineSegVector.getxVec() , startingPointOfLineSegment.getXPos() + lookAheadIndexT2*lineSegVector.getyVec());
              firstLookAheadFound = true;
      ...
      findRobotCurvature();
      setWheelVelocities(chosenPath.getCurveAdjustedVelocity(closestSegment), desiredRobotCurvature);
    }
  } 
  private void setWheelVelocities(double targetVelocity, double curvature){
    ...
    SmartDashboard.putNumber("leftVelcotiy", leftVelocity);
